...
(straight-use-package
 '(fzf-native :type git :host github :repo "dangduc/fzf-native" :files (:defaults "bin")))
  ...
  (defun j-company-capf (f &rest args)
    ...
    (if (length> company-prefix 3)
        ...
        (let ((prescient-filter-method '(fuzzy)))
          (apply f args))
      (let ((prescient-filter-method '(literal-prefix)))
        (apply f args))))
...
(with-eval-after-load 'company
  (add-hook 'lsp-mode-hook
            (lambda () (setq-local company-prescient-sort-length-enable nil)))
